...
{ 2D landscape (without rotation). Made by Bas van Gaalen, Holland, PD }
const
  vseg = $a000;
  a_density = 4;
  roughness = 20;
...
begin
  ...
  for i := 0 to maxx_scape*maxy_scape-1 do landscape[i] := 110+Landscape[i] div 2;
end;

{ the actual displaying of the whole thing! ---------------------------------}

procedure displayscape;
var i,j,previ,prevj,n,k,l : word; x : integer;
begin
  i := 0; j := 0;
  repeat
    {retrace;}
    ...
    for n := 0 to maxx*maxy-1 do begin
      x := -(a_density*(integer(n mod maxx)-(maxx shr 1)-1)*45) div (integer(n div maxx)-45)-90;
      if (x >= -250) and (X <= 60) then begin
        mem[vseg:320*(a_density*integer(n div maxx)-landscape[n mod maxx+previ+(n div maxx+prevj)*maxx_scape])+x] := 0;
        mem[vseg:320*(a_density*integer(n div maxx)-landscape[n mod maxx+i+(n div maxx+j)*maxx_scape])+x] :=
          landscape[(integer(n mod maxx)+i)+(integer(n div maxx)+j)*maxx_scape]-100;
      end;
    end;
  until leftpressed;
end;
