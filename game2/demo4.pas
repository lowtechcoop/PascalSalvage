...
{ SPX library - GeoMorph demo Copyright 1993 Scott D. Ramsay  }

Uses SPX_VGA,SPX_KEY,SPX_GEO,SPX_TXT,SPX_FNC;
...
begin
  ...
  setpageactive(2);
  geo_cnt := loadgmp(path+'demo.gmp',gpic,map);
  loadcolors(path+'demo.pal',pal,256);
  fsetcolors(pal);
  x := 50; y := 50;
end;
...
begin
  ...
  repeat
     ...
     putletter(25,27,5,'Speed = '+st(speed));
     putletter(24,26,255,'Speed = '+st(speed));
     pset(160,100,255);
     pset(161,101,5);
...
procedure showit;
begin
   writeln('SPX library - GeoMorph demo');
