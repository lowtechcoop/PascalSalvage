...
{
  Changes to this program since Turbo Pascal Version 6.0:

  1. File|Open and File|New now create editor windows using the
     Editors unit.  The Edit and Search menus have been added to
     provide complete editor functionality.  Help has been expanded
     accordingly.

  2. Save Desktop now writes a "signature" string as the first part
     of the desktop file.  Load Desktop looks for this signature, and
     if it is not found, an error is reported.  This is the "safe" way
     to read/write configuration files.  (Try TYPE TVDEMO.DSK from the
     DOS prompt).  The application's palette is now saved in the desktop
     file.

  3. A bug which caused the desktop's color not to be set correctly
     has been fixed.
}
...
uses
  ...
  MsgBox, App, DemoCmds, Gadgets, AsciiTab,
...
begin
  ...
  begin
    ...
    Insert(New(PStaticText, Init(R,
      #13 +
      ^C'Blair''s test turbo vision '#13 +
      #13 +
      ^C'hopefully this workes'#13 +
      #13 +
      ^C'Another BlairSoft Production')));
