...
procedure setmode; assembler; asm
  ...
  mov ax,4609h; out dx,ax; mov ax,0194h; out dx,ax; mov ax,0f317h; out dx,ax
...
procedure retrace; assembler; asm
  ...
  @vert2: in al,dx; test al,1; jnz @vert2; end;
...
begin
  ...
  for x:=0 to 63 do begin
    setpal(x,x div 1,x div 2,x);
    setpal(127-x,x div 1,x div 2,x);
    setpal(127+x,x,x div 1,x div 2);
    setpal(254-x,x,x div 1,x div 2);
  end;
  ...
  i1:=55; j1:=199; address:=0;
  repeat
    retrace;
    setborder(30);
    if address=0 then address:=16000 else address:=0;
    setaddress(address);
    inc(i1,-1);
    inc(j1,2);
