...
procedure save;
begin
if gender='m' then player[x].sex:= 0;
if gender='f' then player[x].sex:= 1;
if gender='M' then player[x].sex:= 0;
if gender='F' then player[x].sex:= 1;
...
test(player[account].names);
textcolor(3);
writeln;
writeln(Player[account].weapon_num);
writeln(Player[account].arm_num );
write(Player[account].hit_points);
write('/',Player[account].hit_max);
writeln('');
writeln(Player[account].charm);
writeln(Player[account].weapon);
writeln(Player[account].arm);
writeln(Player[account].exp);
writeln(Player[account].strength);
writeln(Player[account].def);
if Player[account].sex = 1 then gender:='Female';
if Player[account].sex=0 then gender:='Male';
writeln(gender);
end;
procedure news;
...
Player[x].real_names:=real_name;
writeln('alias');
readln(Player[x].names);
writeln('gender');
readln(gender);
Player[x].weapon_num := 1;
Player[x].arm_num :=1;
Player[x].hit_points :=20;
Player[x].hit_max :=20;
Player[x].charm :=1;
Player[x].weapon:='Nuke';
Player[x].arm:='Vest';
Player[x].exp:=0;
Player[x].strength:=0;
Player[x].def:=0;
...
repeat
x:=x+1;
if Player[x].real_names=real_name then account:=x;
until Player[x].real_names='';
if account =0 then new_user;
if account>0 then write('Are You Sure You Want to enter ');
if account>0 then test(player[account].names);
if account >0 then readln(choice);
if choice = 'y' then news;
end;
